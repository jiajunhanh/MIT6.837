...
static void keyCallback(GLFWwindow *window, int key,
  ...
  switch (key) {
    ...
    case 'c':
    case 'C':gCurveMode = (gCurveMode + 1) % 3;
      break;
    case 's':
    case 'S':gSurfaceMode = (gSurfaceMode + 1) % 3;
      break;
    case 'p':
    case 'P':gPointMode = (gPointMode + 1) % 2;
      break;
    default:cout << "Unhandled key press " << key << "." << endl;
  }
}
...
void loadObjects(int argc, char *argv[]) {
  ...
  cerr << endl << "*** loading and constructing curves and surfaces ***"
       << endl;
